...
class Mock {
      ...
      case ts.SyntaxKind.EnumDeclaration: {
        ...
        const enumMenber =
          enumDeclaration.members[
            randomNum(0, enumDeclaration.members.length - 1)
          ];
  ...
  private generateByType(type?: ts.TypeNode, option: MockOptions = {}): any {
    ...
    if (type.getText() === option.$$pre_type)
      option.$$reference = option.$$reference ? option.$$reference + 1 : 1;
  ...
  private generateCustom(
    type?: ts.TypeNode,
    option: CustomMockOptions = { type: "string" },
  ) {
    if (!type) return null;
    switch (option.type) {
      case "string":
  ...
  private generateNumber(option: MockNumberOptions) {
    const {
      min = BASIC_MIN_NUMBER,
      max = BASIC_MAX_NUMBER,
      fixed = BASIC_FIXED,
      type,
    } = option;
    switch (type) {
  ...
  private generateArray(type: ts.TypeNode, option: MockArrayOptions) {
    ...
    const array: any[] = [];
    this.execCount(
      () => array.push(this.generateByType(arrayNode.elementType, {})),
      length,
    );
    return array;
  }
