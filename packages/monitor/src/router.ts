...
export class Router {
  ...
  public changeListeningMode(newMode: TriggeringMethod): void {
    if (Object.values(TriggeringMethod).includes(newMode)) {
      ...
      console.error(
        `[PuzzleRouterError] invalid mode. Proceeding with current mode: ${this.mode}`,
      );
    }
  }
  /**
  ...
  private hijackHistoryApis() {
    ...
    window.history.replaceState = this.createHistoryEvent(
      RouterEvents.REPLACE_STATE,
    );
  }
  ...
  private dispatchUrlChangeEvent(originEvent?: Event) {
    const newEvent: UrlChangeEvent | undefined =
      this.buildUrlChangeEvent(originEvent);
  ...
  private buildUrlChangeEvent(event?: Event): UrlChangeEvent | undefined {
    ...
    try {
      ...
      // 其余时候触发 前提是新旧 url 不等
        ...
        const isPathChanged: boolean =
          newUrlObject.pathname !== oldUrlObject.pathname;
        ...
        if (
          triggeredBy === TriggeringMethod.BOTH ||
          this.mode === TriggeringMethod.BOTH ||
          triggeredBy === this.mode
        ) {
          urlChangeEvent = {
            triggeredBy,
            curUrl: this.buildCustomUrlObject(newUrlObject),
      ...
      this.logger.error(
        `解析 url (${this.curUrl} | ${window.location.href})出错: ${error}`,
      );
    }
    this.curUrl = window.location.href;
