...
export class Monitor {
  ...
  constructor(options: MonitorOptions) {
    ...
    this.options = {
      ...
      inactiveTimeout:
        options.inactiveTimeout || defaultMonitorOptions.inactiveTimeout,
      shouldMonitorElem:
        options.shouldMonitorElem || defaultMonitorOptions.shouldMonitorElem,
  ...
  private monitorActivityEvents() {
    const activeHandler = debounce((e) => {
      ...
      if (single) {
        this.curPage?.setCurMonitorElem(
          e.target.getAttribute("data-monitor-id")!,
        );
      } else if (outerElem) {
        this.curPage?.setCurMonitorElem(
          outerElem.getAttribute("data-monitor-id")!,
        );
      } else {
  ...
  private onPageEnter(event?: UrlChangeEvent) {
    ...
    const savedData = isLocalStorageSupported()
      ? localStorage.getItem(PAGE_STATS_CACHE_KEY)
      : undefined;
    ...
    this.dispatchEvent(
      CustomEvent.ENTER,
      this.curPage?.generatePvEvent(CustomEvent.ENTER),
    );
    !event &&
      this.dispatchEvent(
        CustomEvent.LOAD,
        this.curPage?.generatePvEvent(CustomEvent.LOAD),
      );
  }
  ...
  private handlePageChange(event: UrlChangeEvent) {
    ...
    if (
      this.curPage &&
      this.curPage.currUrl?.[uniqKey] === event.curUrl?.[uniqKey]
    )
      return;
  ...
  private handleHiddenState() {
    ...
    this.dispatchEvent(
      CustomEvent.INACTIVE,
      this.curPage?.generatePvEvent(CustomEvent.INACTIVE),
    );
    this.dispatchEvent(
      CustomEvent.HIDDEN,
      this.curPage?.generatePvEvent(CustomEvent.HIDDEN),
    );

    if (
      [TriggerState.HIDDEN, TriggerState.INACTIVE].includes(
  ...
  private handleTerminatedState() {
    this.dispatchEvent(
      CustomEvent.EXIT,
      this.curPage?.generatePvEvent(CustomEvent.EXIT),
    );
  ...
  private triggerPageViewEvent(event?: MonitorData) {
    this.dispatchEvent(
      ...
      event || this.curPage?.generatePvEvent(CustomEvent.PAGE_VIEW),
    );
    window.clearTimeout(this.inactiveTimerHandle);
  }
  ...
  private cachePageStats() {
    try {
      ...
      value &&
        isLocalStorageSupported() &&
        localStorage.setItem(PAGE_STATS_CACHE_KEY, JSON.stringify(value));
    } catch (error) {
