...
export class Page {
  ...
  public checkElement(elementInfo: ElementInfo, isNew: boolean) {
    if (isNew) {
      ...
      const currentNode = this.monitorElements.find(
        (item) => item.key === elementInfo.key,
      );
  ...
  public onActive() {
    ...
    this.options.monitorElemConfig?.bindVisibilityWithActivity &&
      this.toggleElementsVisible(true, true);
  }
  ...
  public onInActive() {
    ...
    this.isActive = false;
    this.options.monitorElemConfig?.bindVisibilityWithActivity &&
      this.toggleElementsVisible(false, false);
  }
